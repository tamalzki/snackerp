                ...
                <form action="{{ route('expenses.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="category"
                            ...
                            @foreach(['utilities','salaries','rent','supplies','maintenance','transport','other'] as $cat)
                        ...
                        <div class="d-flex gap-3 mb-2">
                            <div class="form-check">
                                <input class="form-check-input" type="radio"
                                       name="paid_from" id="paidCash"
                                       value="cash" checked
                                       onchange="toggleAccounts('cash')">
                                <label class="form-check-label" for="paidCash">
                                    <i class="bi bi-wallet2"></i> Cash Account
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio"
                                       name="paid_from" id="paidBank"
                                       value="bank"
                                       onchange="toggleAccounts('bank')">
                                <label class="form-check-label" for="paidBank">
                            ...
                            <select name="source_id" class="form-select @error('source_id') is-invalid @enderror">
                                ...
                                @foreach($cashAccounts as $a)
                                    <option value="{{ $a->id }}" {{ old('source_id') == $a->id ? 'selected' : '' }}>
                                        {{ $a->name }} — ₱{{ number_format($a->balance, 2) }} available
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="source_id" class="form-select" disabled>
                                ...
                                @foreach($bankAccounts as $a)
                                    <option value="{{ $a->id }}">
                                        ...
                                        (₱{{ number_format($a->balance, 2) }} available)
                                    </option>
                                @endforeach
                            </select>
                        </div>
